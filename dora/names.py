...
class NamesMixin:
    ...
    @classmethod
    def short_name_part(cls, key: str, value: tp.Any) -> str:
    ...
    @classmethod
    def get_name(cls, xp: XP) -> str:
        """Returns the XP name.
        """
        return cls.get_names([xp])[-1]

    @classmethod
    def _get_short_name(cls, parts: OrderedDict, reference: dict = {}):
        out_parts = []
        for key, value in parts.items():
            if key not in reference:
                part = cls.short_name_part(key, value)
                out_parts.append(part)
        return " ".join(out_parts)

    @classmethod
    def get_names(cls, xps: tp.List[XP]) -> tp.Tuple[tp.List[str], str]:
        """Given list of XPs, return individual XP names + base name.
        The common part in all XPs are factored into the base name
        """
        assert len(xps) > 0
        reference = cls.get_name_parts(xps[0])
        all_xp_parts = []
        for xp in xps:
            parts = cls.get_name_parts(xp)
        ...
        for parts in all_xp_parts:
            names.append(cls._get_short_name(parts, reference))

        base_name = cls._get_short_name(reference)
